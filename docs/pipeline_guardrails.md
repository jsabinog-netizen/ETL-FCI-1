...
- La reconciliación registra `reconciled`, `reconcile_blocked`, `reconcile_skipped` o `reconcile_error`. Los fallos de lectura, JSON, IDs o BigQuery se propagan. El guardrail conserva su MERGE sin DELETE cuando bloquea un borrado. El orquestador no ejecuta dbt si hubo módulos fallidos o bloqueados. Los omitidos por origen vacío se resumen con WARNING y permiten continuar a dbt: `Participantes_Bootcamps` de Colsubsidio nunca se usó, según confirmó el área.
